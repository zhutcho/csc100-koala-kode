...
class LoginPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.user_name.place(in_=login_frame, relx=0.5,
                             rely=0.25, anchor=tk.CENTER)
        # Username Label
        username_label = tk.Label(login_frame, text="Username: ")
        username_label.place(in_=self.user_name, relx=.125,
                             rely=-0.75, anchor=tk.CENTER)
        # Password Entry Box
        self.pass_word = tk.Entry(login_frame, width=35)
        self.pass_word.place(in_=login_frame, relx=0.5,
                             rely=0.45, anchor=tk.CENTER)
        self.pass_word.config(show='*')
        # Password Label
        password_label = tk.Label(
            login_frame, text="Password: ")
        password_label.place(in_=self.pass_word, relx=0.125,
                             rely=-0.75, anchor=tk.CENTER)
        # Login Button
        login_button = tk.Button(
            login_frame, text="Login", command=lambda: self.login_entry())
        login_button.place(in_=self.pass_word, relx=0.1,
                           rely=2, anchor=tk.CENTER)
        # Warning Display Label
        self.login_warning_label = tk.Label(login_frame,
                                            text="", fg='#C03A3A')
        self.login_warning_label.place(
            in_=login_frame, relx=0.28, rely=0.7, anchor=tk.W)
